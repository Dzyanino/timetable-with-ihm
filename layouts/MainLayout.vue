  ...
  <v-app>
    ...
    >
      <template v-slot:prepend>
        ...
        <v-app-bar-title>
          <v-btn variant="plain" class="text-no-wrap text-green" link to="/"
            >Fotoana</v-btn
          >
        </v-app-bar-title>
      </template>
      ...
      <div class="d-none d-md-flex">
        <template v-for="postLink in navBarLink[1]" :key="postLink.to">
          <v-btn
            size="small"
            :prepend-icon="'mdi-' + postLink.icon"
            :text="postLink.label"
            :to="postLink.to"
            class="mx-1"
          />
    ...
    <v-navigation-drawer temporary v-model="nav">
      ...
      <template v-slot:append>
        <v-divider />
        <template
          v-for="postLink in navBarLink[1]"
          :key="postLink.to"
          class="d-flex d-md-none"
        >
          <v-list-item
